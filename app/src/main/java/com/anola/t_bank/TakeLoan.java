...
public class TakeLoan extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_take_loan);
        preferences = getSharedPreferences("myPrefs", MODE_PRIVATE);
        loanField = findViewById(R.id.myLoan);
        proBtn = findViewById(R.id.loanBtn);


        proBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String amount = loanField.getText().toString();
                //String userText  = username.getText().toString();
                // String passText  = password.getText().toString();
                RequestBody requestBody = new MultipartBody.Builder()
                        .setType(MultipartBody.FORM)
                        .addFormDataPart("loan_amt", amount)
                        .build();
                new TakeLoan.Mybackground().execute(requestBody);
            }
        });
    }
    ...
    class Mybackground extends AsyncTask<RequestBody, Void, String> {
        ...
        protected String doInBackground(RequestBody... voids) {
            ...
            Request request = new Request.Builder()
                    .url("http://192.168.1.31:8084/api/loanuser?access_token="+access_token)
                    .post(requestBody)
                    .build();
            try{
        ...
        protected void onPostExecute(String result) {
            if(result.equals("Minimum_issue")){
                ...
                Toast.makeText(TakeLoan.this,"Operation Failed, Your Account is too low to complete the transaction",Toast.LENGTH_LONG).show();
            }else if(result.equals("Successful")){
                Toast.makeText(TakeLoan.this,"Operation Successful",Toast.LENGTH_LONG).show();
                loanField.setText("0.0");
