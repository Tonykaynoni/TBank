...
public class CreditAccount extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        crdBtn = findViewById(R.id.creditBtn);
        credField = findViewById(R.id.creditField);


        crdBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String amount = credField.getText().toString();
                //String userText  = username.getText().toString();
               // String passText  = password.getText().toString();
                RequestBody requestBody = new MultipartBody.Builder()
                        .setType(MultipartBody.FORM)
                        .addFormDataPart("credit_amount", amount)
                        .build();
                new CreditAccount.Mybackground().execute(requestBody);
            }
        });
    }
    ...
    class Mybackground extends AsyncTask<RequestBody, Void, String> {
        ...
        protected String doInBackground(RequestBody... voids) {
            ...
            Request request = new Request.Builder()
                    .url("http://192.168.1.31:8084/api/credit_acct?access_token="+access_token)
                    .post(requestBody)
                    .build();
            try{
        ...
        protected void onPostExecute(String result) {
            if(result.equals("Minimum_issue")){
                ...
                Toast.makeText(CreditAccount.this,"Operation Failed, You must add an amount greater than your minimum allowed amount",Toast.LENGTH_LONG).show();
            }else if(result.equals("Successful")){
                Toast.makeText(CreditAccount.this,"Operation Successful",Toast.LENGTH_LONG).show();
